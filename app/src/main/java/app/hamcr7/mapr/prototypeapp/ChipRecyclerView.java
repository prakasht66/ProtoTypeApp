...
public class ChipRecyclerView extends RecyclerView {
    ...
    public ChipRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, com.ileaf.chiprecyclerviewlibrary.R.styleable.ChipRecyclerView, 0, 0);
        try {
            isMultiChoice = ta.getBoolean(com.ileaf.chiprecyclerviewlibrary.R.styleable.ChipRecyclerView_multiSelection, false);
        } finally {
            ta.recycle();
        }
        setHorizontalRecyclerView();
